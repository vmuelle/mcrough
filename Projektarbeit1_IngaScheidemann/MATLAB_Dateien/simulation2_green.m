...
model = plot(model,'MC');
green = model.MC.normalizedFluenceRate(:,:,:);
...
function mediaProperties = mediaPropertiesFunc(parameters)
  ...
  mediaProperties(j).mua   = 8.73; % Absorption coefficient in cm^-1
  ...
  mediaProperties(j).mua   = 8.52; % Absorption coefficient in cm^-1
  mediaProperties(j).mus   = 77.22; % Scattering coefficient in cm^-1
  mediaProperties(j).g     = 0.8; % Henyey-Greenstein scattering anisotropy
  mediaProperties(j).n     = 1.4; % Refractive index
