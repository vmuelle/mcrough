...
model.MC.wavelength               = 570; % Excitation wavelength in nm, used for determination of optical properties for excitation light
...
model = plot(model,'MC');
green_systole = model.MC.normalizedFluenceRate(:,:,:);
save('green_systole.mat','green_systole');
...
function mediaProperties = mediaPropertiesFunc(parameters)
  ...
  mediaProperties(j).mua   = ((0.1-0.3*10^-4*lambda)+0.125*mua_baseline)*(1-SC_Vw)*mua_w_530; % Absorption coefficient in cm^-1
  mediaProperties(j).mus   = 74.7; % Scattering coefficient in cm^-1
  mediaProperties(j).g     = 0.923; % Henyey-Greenstein scattering anisotropy
  mediaProperties(j).n     = 1.5; % Refractive index

  j=3;
  LE_Vmel = 0.11; % fraction of melanin
  LE_Vw = 0.2; % fraction of water
  mua_mel_530 = 559.4; % absorption coefficient for melanin at 530 nm in cm^-1
  mediaProperties(j).name  = 'Living Epidermis';
  mediaProperties(j).mua   = (LE_Vmel*mua_mel_530+(1-LE_Vmel)*mua_baseline)*(1-LE_Vw)+LE_Vw*mua_w_530; % Absorption coefficient in cm^-1
  mediaProperties(j).mus   = 74.7; % Scattering coefficient in cm^-1
  mediaProperties(j).g     = 0.8; % Henyey-Greenstein scattering anisotropy
  mediaProperties(j).n     = 1.34; % Refractive index

  j=4;
  V_A_SP = 0.2; % fraction of arterial blood
  ...
  mu_a_HbO2 = 243.8; % absorption coefficient oxygenated blood in cm^-1
  mu_a_HHb = 232.9; % absorption coefficient deoxygenated blood in cm^-1
  ...
  mediaProperties(j).mua   = 4.15; % Absorption coefficient in cm^-1
  mediaProperties(j).mus   = 85.2; % Scattering coefficient in cm^-1
  mediaProperties(j).g     = 0.7949; % Henyey-Greenstein scattering anisotropy
  mediaProperties(j).n     = 1.41; % Refractive index

  j=8;
  mediaProperties(j).name  = 'Knochen';
  mediaProperties(j).mua   = 1.004; % Absorption coefficient in cm^-1
