...
model = plot(model,'MC');
IR_systole = model.MC.normalizedFluenceRate(:,:,:);
save('IR_systole.mat','IR_systole');
...
function mediaProperties = mediaPropertiesFunc(parameters)
  ...
  g_SC = 0.942; % anisotropyfactor for SC at 940 nm
...
% mediaProperties(j).mua   = ((0.1-0.3*10^-4*lambda)+0.125*mua_baseline)*(1-SC_Vw)*mua_w_940; % Absorption coefficient in cm^-1
  ...
  g_LE = 0.8; % anisotropyfactor for LE at 940 nm
...
% mediaProperties(j).mua   = (LE_Vmel*mua_mel_940+(1-LE_Vmel)*mua_baseline)*(1-LE_Vw)+LE_Vw*mua_w_940; % Absorption coefficient in cm^-1
  ...
  mu_a_HbO2 = 6.5; % absorption coefficient oxygenated blood in cm^-1
  mu_a_HHb = 4.3; % absorption coefficient deoxygenated blood in cm^-1
  mu_a_A = SaO2*mu_a_HbO2+(1-SaO2)*mu_a_HHb;
  mu_a_V = SvO2*mu_a_HbO2+(1-SvO2)*mu_a_HHb;
  mediaProperties(j).name  = 'Stratum Papillare';
  mediaProperties(j).mua   = V_A_SP*mu_a_A+V_V_SP*mu_a_V+V_w_SP*mua_w_940+(1-V_A_SP+V_V_SP+V_w_SP)*mua_baseline; % Absorption coefficient in cm^-1
  ...
  mediaProperties(j).mua   = V_A_SR*mu_a_A+V_V_SR*mu_a_V+V_w_SR*mua_w_940+(1-V_A_SR+V_V_SR+V_w_SR)*mua_baseline; % Absorption coefficient in cm^-1
